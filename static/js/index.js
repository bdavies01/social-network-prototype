...
let init = (app) => {
    ...
    app.index = (a) => {
        ...
        for (let p of a) {
            ...
            //p.editable = true; COMMENTED OUT BY ME
            p.edit = false;
            p.is_pending = false;
            p.error = false;
    ...
    app.do_edit = (post_idx) => {
        ...
        for(index = 0; index < app.vue.posts.length; index++) {
            if(app.vue.posts[index].edit == true) {
                return;
            }
        }
    ...
    app.init = () => {
        ...
        // We set the posts.
        // app.vue.posts = app.index(posts); THIS HAS BEEN COMMENTED OUT BY ME
    };
