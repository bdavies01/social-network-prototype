...
def get_posts():
    ...
    mainposts = db(db.post.is_reply == None).select(orderby=~db.post.post_date).as_list()
    replies = db(db.post.is_reply != None).select(orderby=~db.post.post_date).as_list()
    for p in mainposts:
        pappend.append(p)
        for r in replies:
            if r["is_reply"] == p["id"]:
    ...
    for p in pappend:
        ...
        p["author"] = name
        if p["email"] == auth.current_user.get("email"):
            p["editable"] = True
    return dict(posts=pappend)
